...
import transformers
from transformers.trainer_pt_utils import LabelSmoother, get_model_param_count
from transformers import Trainer, TrainerCallback
from model.configuration_yi import YiConfig
from model.modeling_yi import YiForCausalLM
from model.tokenization_yi import YiTokenizer
...
def copy_modeling_files(model_name_or_path, output_dir):
    subdirs = [
        os.path.join(output_dir, subdir) for subdir in os.listdir(output_dir) if
        os.path.isdir(os.path.join(output_dir, subdir))]
    checkpoint_subdirs = [
        subdir for subdir in subdirs if subdir.startswith(os.path.join(output_dir, 'checkpoint-'))]
    logger.info(f"Copy modeling files to:")
    for checkpoint_subdir in checkpoint_subdirs:
        cmd = f'cp {model_name_or_path}/*.py {checkpoint_subdir}'
        logger.info(cmd)
        os.system(cmd)
...
def train():
    ...
    config = YiConfig.from_pretrained(
    ...
    copy_modeling_files(model_args.model_name_or_path, output_dir)
