...
"""
We inherit fastchat's Conversation prompt templates.
"""
import copy
import json
import dataclasses
...
class Conversation:
    ...
    b_gatt: str = "[INST]"
    e_gatt: str = "[/INST]"
    # Assitant self-aware role
    assistant_role: str = "assistant"
    assistant_role_types: List[dict] = ("bot_query", "bot_action")
    formatted_prompt: str = None
    formatted_turns: List[dict] = None

    def set_assistant_role(self):
        if self.roles and len(self.roles) > 1:
            self.assistant_role = self.roles[1]
    ...
    def get_role_content_type(self, index):
        if index < 0 or index >= len(self.messages):
            raise ValueError(f"index {index} out of messages length {len(self.messages)}")
        messages_tuple = self.messages[index]
        if len(messages_tuple) == 3:
            (role, message, vtype) = messages_tuple
        else:
            (role, message) = messages_tuple
            vtype = "bot_query" if self.is_assistant_turn(role) else "user_query"

        return role, message, vtype

    def is_bot_message(self, index):
        role, message, vtype = self.get_role_content_type(index)
        return self.is_assistant_turn(role, vtype)

    def is_bot_formatted_turn(self, index):
        if index < 0 or index >= len(self.formatted_turns):
            raise ValueError(f"index {index} out of formatted_turns length {len(self.formatted_turns)}")
        role = self.formatted_turns[index].get("from", None)
        vtype = self.formatted_turns[index].get("type", None)
        return self.is_assistant_turn(role, vtype)
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, messages_tuple in enumerate(self.messages):
                role, message, vtype = self.get_role_content_type(i)
                if message:
            ...
            for role, message in self.messages:
                if message:
                    ...
                    ret += role + "\n"
            return ret
        elif self.sep_style == SeparatorStyle.MTAGENT:
            ret = ""
            self.formatted_prompt = self.get_formatted_system()
            ret += self.formatted_prompt

            for i, messages_tuple in enumerate(self.messages):
                role, message, vtype = self.get_role_content_type(i)

                if message and (vtype not in self.assistant_role_types):
                    if self.gatt and self.system_message:
                        value = self.b_gatt + self.sep + f"<<{role}>>" + "\n" + message + self.sep + self.e_gatt + "\n\n"
                    else:
                        value = f"<<{role}>>" + "\n" + message + "\n\n"

                    if i == 0 or (self.get_role_content_type(i - 1)[-1] in self.assistant_role_types):
                        value = self.bos + value

                elif message:  # assistant turn
                    value = f"<<{role}>>" + "\n" + message + self.eos + "\n\n"
                else:
                    value = f"<<{role}>>" + "\n"

                ret += value
                self.formatted_turns.append(
                    {
                        "from": role,
                        "value": value,
                        "type": vtype,
    ...
    def clear_message_buffer(self):
        """Clear the message buffer for new sample."""
        self.messages = []
        self.formatted_turns = []
    ...
    def get_formatted_system(self):
        """Get formatted_prompt."""
        if self.sep_style in (
                SeparatorStyle.MTAGENT,
        ):
            ret = ""
            if self.gatt and self.system_message:
                ret = self.b_gatt + self.sep + self.system_message + self.sep + self.e_gatt + self.eos + "\n" + "\n"
            elif self.system_message:
                ret = self.system_message + self.eos + "\n" + "\n"
            self.formatted_prompt = ret
        else:
            self.formatted_prompt = self.system_template.format(system_message=self.system_message)
        return self.formatted_prompt

    def get_formatted_turns(self):
        """Get formatted_turns."""
        self.get_prompt()
        return copy.deepcopy(self.formatted_turns)
    ...
    def to_minimax_api_messages(self):
        ...
        {"sender_type": "USER", "sender_name": "用户", "text": "你有几个徒弟来着，最喜欢哪一个？？"}
        """
        ret = []
        for i, messages_tuple in enumerate(self.messages[self.offset :]):
            if len(messages_tuple) == 3:
                ...
                raise ValueError(f"Minimax's chat messages required three elements, but got {len(messages_tuple)}")
            if vtype.lower().startswith("user"):
                ret.append({"sender_type": "USER", "sender_name": role, "text": message})
...
def get_conv_template(name: str) -> Conversation:
    ...
    _template = conv_templates[name].copy()
    _template.formatted_turns = []
    return _template
...
register_conv_template(
    # Works best for llama2, Baichuan2
    Conversation(
        name="mtagent_eos",
        roles=("user", "assistant"),
        sep_style=SeparatorStyle.MTAGENT,
        sep=" ",
        eos="\n</s>",
...
if __name__ == "__main__":
    template_id = "mtagent"
    conv = get_conv_template(template_id)
    print(conv)
